...
app.post("/api/friend/invite", auth, async (req, res) => {
  try {
    ...
    if (friend._id.equals(req.user._id)) return res.status(400).json({ error: "Cannot add yourself" });
    if (req.user.friends.includes(friend._id)) return res.status(400).json({ error: "Already friends" });
    if (friend.friendRequests.includes(req.user._id)) return res.status(400).json({ error: "Already invited" });
...
app.get("/api/friend/requests", auth, async (req, res) => {
  await connectToDatabase();
  const requests = await User.find({ _id: { $in: req.user.friendRequests } }).select("name phone");
...
app.post("/api/friend/accept", auth, async (req, res) => {
  try {
    ...
    if (!req.user.friendRequests.includes(friend._id)) return res.status(400).json({ error: "No such pending invite" });

    req.user.friends.push(friend._id);
    friend.friends.push(req.user._id);
    req.user.friendRequests = req.user.friendRequests.filter((id) => !id.equals(friend._id));
...
app.post("/api/friend/reject", auth, async (req, res) => {
  try {
    await connectToDatabase();
    const { friendId } = req.body;
    if (!req.user.friendRequests.includes(friendId)) return res.status(400).json({ error: "No such pending invite" });
...
app.post("/api/group/:groupId/expense", auth, async (req, res) => {
  ...
  const expense = await Expense.create({ label, amount, date: date || new Date(), group: group._id, splits, paid: false });
...
app.get("/", async (req, res) => {
  ...
  try { await connectToDatabase(); mongoStatus = "✅ MongoDB connected"; } catch {}
